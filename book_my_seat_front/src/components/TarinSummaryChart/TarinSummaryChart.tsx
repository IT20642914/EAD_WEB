...
import style from "./TarinSummaryChart.module.scss";
import {
  Box,
  Card,
  CardContent,
...
const TrainSummaryChart = (props: any) => {
  const [totalRequests, setTotalRequests] = useState<number>(0);

  // eslint-disable-next-line react-hooks/exhaustive-deps
  const data = {
    labels: ["Express", "Inter-City","Slow" ],
    datasets: [
      {
        label: "No of Trains",
        data:[10,16,15],
    ...
    <Card className={style.summaryCard}>
      <CardContent>
        <Box className={style.summaryCardTitle}>
          <Box>
            <Typography className={style.main}>Active Trains Summary</Typography>
        ...
        <>
          <Box className={style.CardBox}>
            <Box sx={{ display: "flex", justifyContent: "space-between" }}>
              ...
              <Box
                sx={{ display: "flex", height: "9rem", marginTop: "0.2rem" }}
              >
...
export default TrainSummaryChart;
